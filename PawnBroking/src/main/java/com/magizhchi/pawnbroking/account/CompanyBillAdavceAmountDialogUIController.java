 ...
 * @author Tiru
 */
public class CompanyBillAdavceAmountDialogUIController implements Initializable {
    ...
    @FXML
    private TableView<CompanyBillAdvanceAmountAllDetailsBean> tbTodaysAccount;
        ...
        if(event.getClickCount() == 2 && (index >= 0) ) 
        {
            ...
            if("OPENED".equals(sStatus) || "LOCKED".equals(sStatus)) 
            {
                if(isGoldOperation) {
                    ...
                    gon.closeBill(tbTodaysAccount.getItems().get(index).getSBillNumber(), true);
                    gon.tpScreen.getSelectionModel().select(gon.tabAdvanceAmountDetails);
    ...
    public void setInitValues(DataTable values) {
        ...
        for(int i=0; i<values.getRowCount(); i++) {            
            ...
            String sDate = values.getRow(i).getColumn(1).toString();
            String sBillNumber = values.getRow(i).getColumn(2).toString();
            String sStatus = values.getRow(i).getColumn(3).toString();
            String sAmount = values.getRow(i).getColumn(4).toString();
            String sPaidAmount = values.getRow(i).getColumn(5).toString();
            String sTotalAmount = values.getRow(i).getColumn(6).toString();
            String sCreatedUser = values.getRow(i).getColumn(7).toString();
            tbTodaysAccount.getItems().add(new CompanyBillAdvanceAmountAllDetailsBean(sSlNo, sDate, sBillNumber, sStatus, sAmount, sPaidAmount, sTotalAmount, sCreatedUser));
        }
    }
}
