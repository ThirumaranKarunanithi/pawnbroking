 ...
 * @author Tiru
 */
public class CompanyBillClosingDialogUIController implements Initializable {
    ...
    @FXML
    private TableView<CompanyBillClosingAllDetailsBean> tbTodaysAccount;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tbTodaysAccount.setRowFactory(tv -> new TableRow<CompanyBillClosingAllDetailsBean>() {
            @Override
            public void updateItem(CompanyBillClosingAllDetailsBean item, boolean empty) {
                super.updateItem(item, empty) ;
                if (item == null) {
                    setStyle("");
                } else if (item.getDTotAdvanceAmtPaid() > 1) {
    ...
    public void setParent(TodaysAccountController parent, boolean isGoldOperation)
    {
        this.parent = parent;
    ...
    public void setInitValues(DataTable values) {
        ...
        for(int i=0; i<values.getRowCount(); i++) {            
            ...
            String sCreatedDate = values.getRow(i).getColumn(9).toString();
            String sCreatedUser = values.getRow(i).getColumn(10).toString();
            String sInterest = values.getRow(i).getColumn(11).toString();            
            String sInterestedAmt = values.getRow(i).getColumn(12).toString();
            String sTotAdvAmt = values.getRow(i).getColumn(13) != null ? values.getRow(i).getColumn(13).toString() : "0";
            tbTodaysAccount.getItems().add(new CompanyBillClosingAllDetailsBean(sSlNo, sDate, sBillNumber, 
                    sName, sItems, sInterest, sAmount, sInterestedAmt, 
                    sToGive, sGiven, sCreatedDate, sCreatedUser, Double.parseDouble(sTotAdvAmt)));
        }
    }
}
