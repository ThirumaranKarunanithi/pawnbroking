...
public class AvailableBalanceDialogUIController implements Initializable {
    ...
    public BillCalculatorController billCalcParent;
    private boolean forAdd = true;
    private String getOrGive;
    ...
    public void setInitValues(BillCalculatorController parent, DenominationBean denomBean) throws SQLException {

        this.billCalcParent = parent;
        this.forAdd = true;
        this.getOrGive = parent.lbAmtToGet.getText();
        this.toGetAmt = parent.txtAmtToGet.getText();
        this.sLastSelectedId = denomBean.getBillNumbers();
        this.dbOp = parent.dbOp;
        this.sOperation = denomBean.getOperationName();
        this.sMaterialType = denomBean.getMaterialTypes();
        
        this.isMultiClose = denomBean.isIsMultiClose();
        this.isMultiOperation = denomBean.isIsMultiOperation();
        ...
        if(forAdd) {
            ...
            txtCashGotAmt.textProperty().addListener(new ChangeListener<String>() {
                ...
                public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                    if (!newValue.matches("\\d{0,9}([\\.]\\d{0,2})?")) {                        
                        ...
                        if(getOrGive.equals("Amount to Get:")) {
                            if(dDeficitAmt <= 0) {                            
                        ...
                        } else if(getOrGive.equals("Amount to Give:")) {
                            if(dDeficitAmt <= 0) {
                                lbCashBalance.setText("Balance To Give:");
                                txtCashBalanceAmt.setText(Double.toString(dDeficitAmt).replace("-", ""));
                            } else {
            ...
            if(denomBean.getCurrencyList() == null) {
                ...
                currencyList = denomBean.getCurrencyList();
            }
    ...
    public void setValuesToFileds(int index, int numberOfNotes) {
        ...
        lbToGetAmt.setText(getOrGive);
        if(getOrGive.contains("Give")) {
            lbGotAmt.setText("Given Amount:");
        } else if(getOrGive.contains("Get")) {
            ...
            try {                
                ...
                if(this.dbOp.deleteDenomination(sOperation, sBillNumber, sMaterialType)) {
                    if(this.dbOp.saveDenominationValues(sOperation, sBillNumber, sMaterialType, currencyListToSave)) {                               
                        if(isMultiClose) {
                            this.billCalcParent.doOperationForMultiClose();
                        } else if(isMultiOperation) {
                            this.billCalcParent.doOperationForMultiOperation(event);
                        }
                        //this.billCalcParent.dialog.close(); 
                    }
                }
            } catch (Exception ex) {
