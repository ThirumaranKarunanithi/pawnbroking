 ...
 * @author Tiru
 */
public class GoldAdvanceAmountController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            if(dbOp.getAddViewUpdate(CommonConstants.ROLEID, CommonConstants.OPERATION_TAB, CommonConstants.GOLD_ADVANCE_AMOUNT_SCREEN, "ALLOW_VIEW") || CommonConstants.ROLEID.equals(CommonConstants.TIRU)) {
        ...
        try {
            if(dbOp.getAddViewUpdate(CommonConstants.ROLEID, CommonConstants.OPERATION_TAB, CommonConstants.GOLD_ADVANCE_AMOUNT_SCREEN, "ALLOW_UPDATE") || CommonConstants.ROLEID.equals(CommonConstants.TIRU)) {
    ...
    private void txtBillNumberOnAction(ActionEvent event) {
        ...
        try {
            HashMap<String, String> headerValues = dbOp.getAllBillingValuesToPay(sBillNumber, "GOLD");
            ...
            if(headerValues != null)
            {
                ...
                btEditDuePaid.setDisable(false);
                btSaveDuePaid.setDisable(false);
 ...
 public void setAllHeaderValuesToFields(HashMap<String, String> headerValues, String sInterestType, String[] sReduceDatas, String[] sMinimumDatas)
    {
        try {
            ...
            if("MONTH".equals(sInterestType)) {
                ...
                if("MONTHS FROM TOTAL MONTH".equals(sReduceDatas[1])) {
                    try {
                        ...
                        double dRemainingDaysAsMonths = (Double.valueOf(lActualTotalMonths[0]) > 0) ? dbOp.getRemainingDaysAsMonths(Double.valueOf(lTakenMonths[1]), "GOLD") : 0;
            ...
            String sFormula = dbOp.getFormula(Double.parseDouble(headerValues.get("AMOUNT")), "GOLD");
            ...
            if(DateRelatedCalculations.isFirstDateIsLesserToSecondDate(CommonConstants.ACTIVE_COMPANY_ACC_LAST_DATE, sPaidDate))
            {
                if(DateRelatedCalculations.isFirstDateIsLesserOrEqualToSecondDate(CommonConstants.DATETIMEFORMATTER.format(dpBillOpeningDate.getValue()), sPaidDate))
                {
                    ...
                    if(isValidHeaderValues(dPaidAmount)) {
                        try {
                            if(dbOp.saveRecord(sLastSelectedId, sPaidDate, dBillAmount, dPaidAmount, dTotalAmount, "GOLD")) {
                                if(dbOp.updateTotalAdvanceAmount(sLastSelectedId, txtNote.getText(), dTotalAmount, "GOLD")) { 
    ...
    private void dpPaidDateTextChanged(ActionEvent event) {
        if(sLastSelectedId != null) {            
            try {
                HashMap<String, String> headerValues = dbOp.getAllBillingValuesToPay(sLastSelectedId, "GOLD");
                String sInterestType = dbOp.getInterestType();
                String[] sReduceDatas = dbOp.getReduceOrMinimumDaysOrMonths("GOLD", "REDUCTION");
                String[] sMinimumDatas = dbOp.getReduceOrMinimumDaysOrMonths("GOLD", "MINIMUM");
    ...
    private void btClearAllClicked(ActionEvent event) {
        ...
        sReduceType = null;
        sMinimumType = null;
        btEditDuePaid.setDisable(true);
        btSaveDuePaid.setDisable(true);
    }
    ...
    private void btEditDuePaidClicked(ActionEvent event) {
        ...
        if(index >= 0) 
        {
            ...
            if(DateRelatedCalculations.isFirstDateIsLesserToSecondDate(CommonConstants.ACTIVE_COMPANY_ACC_LAST_DATE, sPaidDate))
            {
                ...
                FXMLLoader loader = new FXMLLoader(getClass().getResource("GoldAdvanceAmountEditDialog.fxml"));
                Parent root = null;
                try {            
                ...
                GoldAdvanceAmountEditDialogUIController gon = (GoldAdvanceAmountEditDialogUIController) loader.getController();
    ...
    private void btSaveDuePaidClicked(ActionEvent event) {
        ...
        try {
            if(dbOp.deleteAllSelectedBillAdvAmtVals(sBillNumber, "GOLD")) {
                if(dbOp.saveBillAdvAmtVals(sBillNumber, tableValues, "GOLD")) {
                    if(dbOp.updateTotalAdvanceAmount(sLastSelectedId, txtNote.getText(), dTotalAmount, "GOLD")) { 
                        txtBillNumber.setText(sLastSelectedId);
                        txtBillNumberOnAction(null);                        
    ...
    private void txtGotAdvanceAmountOnAction(ActionEvent event) {
        ...
        try {
            ...
            List<AvailableBalanceBean> currencyList =
                    dbOp.getDenominationValues(CommonConstants.G_ADVANCE_AMOUNT_OPERATION, 
