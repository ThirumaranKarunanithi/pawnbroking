...
public class AvailableBalanceDialogUIController implements Initializable {
    ...
    public void setParent(GoldBillClosingController parent, boolean forNew)
    {
        this.gParent = parent;
        this.forAdd = forNew;
        this.toGetAmt = parent.txtGotAmount.getText();
        this.sLastSelectedId = parent.sLastSelectedId;
        this.dbOp = parent.dbOp;
        this.sOperation = CommonConstants.G_BILL_CLOSING_OPERATION;
        this.sMaterialType = "GOLD";
    }

    public void setParent(SilverBillClosingController parent, boolean forNew)
    {
        this.sParent = parent;
        this.forAdd = forNew;        
        this.toGetAmt = parent.txtGotAmount.getText();    
        this.sLastSelectedId = parent.sLastSelectedId;
        this.dbOp = parent.dbOp;
        this.sOperation = CommonConstants.S_BILL_CLOSING_OPERATION;
    ...
    public void setInitValues(List<AvailableBalanceBean> existingCurrencyList, 
        ...
        if(isMultiClose) {
            ...
            for(int i=0; i<billNumbers.length; i++) {
                totalAmtToGet = totalAmtToGet + dbOp.getReceivedAmount(billNumbers[i], materialTypes[i]);
            }
            toGetAmt = Double.toString(totalAmtToGet);
        }
        ...
        if(isMultiOperation) {
            ...
            if(totalAmtToGet <= 0) {
                ...
                txtCashToGetAmt.setText(toGetAmt.replace("-", ""));
                txtToGetAmt.setText(toGetAmt.replace("-", ""));
            } else {
            ...
            double deficit = Double.parseDouble(txtGotAmt.getText()) - Double.parseDouble(txtToGetAmt.getText());
