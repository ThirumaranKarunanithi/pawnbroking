...
public class ReBillRecievedAndBalanceAmtDialog implements Initializable {
    ...
    // Pre-computed closing total stored here so setAmountRelatedText never reads stale text fields
    private double dClosingAmountTotal = 0.0;
    ...
    private BillClosingDBOperation closedbOp;
    ...
    @FXML
    private TextField txtExistingWeight;
    @FXML
    private TextField txtTotalWeight;
    @FXML
    private TextField txtNewWeight;
    @FXML
    private HBox hbAddOrRemove;
    @FXML
    private Label lbNewWt;
                ...
                dialogpanel.addEventFilter( KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>()
                                {
                                    ...
                                    public void handle( KeyEvent KV )
                                    {
                                        switch ( KV.getCode() )
                                        {
                                            ...
                                            {
                                                if(KV.isControlDown()) {
                                                    txtNewWeight.requestFocus();
                                                    txtNewWeight.positionCaret(txtNewWeight.getText().length());
                                                }
                                                break;
                                            }                                            
                                            case A :
    ...
    public void setInitValues(boolean forNew, String status, String billNumbers) {
        ...
        if(this.forAdd) {
            if(gParent != null) {
                try {
                    ...
                    sCloseInterestAmt = this.gParent.txtCloseInterest.getText();
                    sOldAmt = this.gParent.txtCloseCapital.getText();
                    sNewAmt = this.gParent.tgOn.isSelected() 
                            ? this.gParent.txtOpenCapital.getText() 
                            : Double.toString(
                                    this.gParent.dbOp.getRebilledToVals("GOLD", this.gParent.txtReBilledTo.getText()));
                    sDiscount = "0";
                    sClosedGotAmt = this.gParent.txtCloseInterested.getText();

                    double dTotAdvAmt = 0;
                    double dTotOtherChrgs = 0;
                    for(com.magizhchi.pawnbroking.billcalculator.AllDetailsBean bean : this.gParent.tbClosingList.getItems()) {                        
                        if(bean.getSOperation().equals("BILL CLOSING")) {                            
                            String[] arg = bean.getSCombo().split(",");
                            dTotOtherChrgs += Double.parseDouble(arg[0].substring(4));
                            dTotAdvAmt += Double.parseDouble(arg[1].substring(4));
                        }
                    }
                    sTotalAdvAmt = Double.toString(dTotAdvAmt);
                    sTotalOtherCharges = Double.toString(dTotOtherChrgs);
                    ...
                    this.sLastSelectedId = billNumbers;
                    this.sOperation = CommonConstants.G_REBILL_CLOSE_OPERATION;
                    this.closedbOp = this.gParent.dbOp;

                    double dGrossWt = Double.parseDouble(sGrossWeight.isEmpty() ? "0" : sGrossWeight);
                ...
                try {
                    sTotalAdvAmt = this.sParent.txtTotalAdvanceAmountPaid.getText();
                    sCloseInterestAmt = this.sParent.txtTakenAmount.getText();
                    sOldAmt = this.sParent.txtAmount.getText();
            ...
            if(gParent != null) {
                try {
                    HashMap<String, String> headerValues = this.gParent.dbOp.getRebilledFromVals("GOLD", billNumbers);
                    ...
                    sPurity = (headerValues.get("purity") != null && !headerValues.get("purity").isEmpty())
                            ? headerValues.get("purity") : this.gParent.txtPurity.getText();
                    sReductionRate = this.gParent.dbOp.getReductionWt("GOLD");
                    sCompanyRate = this.gParent.dbOp.getCompanyRate("GOLD");
                    this.sLastSelectedId = billNumbers;
                    this.sOperation = CommonConstants.G_REBILL_CLOSE_OPERATION;                
        ...
        if((this.gParent != null && !this.gParent.tgOn.isSelected())
                || (this.sParent != null && !this.sParent.tgOn.isSelected())) {
        ...
        if(status.equals("REBILLED") || status.equals("REBILLED-MULTIPLE")) {
            hbAddOrRemove.getChildren().removeAll(hbAddOrRemove.getChildren());
            Platform.runLater(() -> {
                txtOpenNewAmt.requestFocus();
                txtOpenNewAmt.positionCaret(txtOpenNewAmt.getText().length());
            });        
        } else {
            if(status.equals("REBILLED-ADDED")) {
                lbNewWt.setText("Newly Added Weight:");
            } else if(status.equals("REBILLED-REMOVED")) {
                lbNewWt.setText("Newly Removed Weight:");
            }   
            txtExistingWeight.setText(sGrossWeight);
            txtTotalWeight.setText(sGrossWeight);
            Platform.runLater(() -> {
                txtNewWeight.requestFocus();
                txtNewWeight.positionCaret(txtNewWeight.getText().length());
            });                    
    ...
    public void setInitValues(boolean forNew, String status) {
        ...
        if(gParent != null) {
            try {
                sTotalAdvAmt = this.gParent.txtTotalAdvanceAmountPaid.getText();
                sCloseInterestAmt = this.gParent.txtTakenAmount.getText();
                sOldAmt = this.gParent.txtAmount.getText();
                sNewAmt = this.gParent.tgOn.isSelected() ? this.gParent.txtToGetAmount.getText() 
    ...
    public void setValuesToFileds(int index, int numberOfNotes) {
        ...
        for(TextField txtTotAmt : lstTotalAmt) {
            String v = txtTotAmt.getText();
            totAmt = totAmt + (v == null || v.isEmpty() ? 0 : Double.parseDouble(v));
        }
    ...
    public void setParent(GoldBillClosingController gParent)
    {
        this.gParent = gParent;
        this.sMaterialType = "GOLD";
    }

    public void setParent(SilverBillClosingController sParent)
    {
    ...
    private void txtAmountOnType(KeyEvent e) {
        ...
        // Use runLater so the field text is read AFTER the character is actually inserted
        Platform.runLater(() -> {
    ...
    private void txtAmountOnPress(KeyEvent e) {
        if (e.getCode() == KeyCode.BACK_SPACE) {
            // Use runLater so the field text is read AFTER backspace has been applied
            Platform.runLater(() -> {
                String sAmount = txtOpenNewAmt.getText().trim();
                if (!sAmount.isEmpty()) {
                    try {
    ...
    private double safeDouble(String s) {
        if (s == null || s.trim().isEmpty() || "NaN".equals(s)) return 0.0;
    ...
    public void setAmountRelatedText(double dAmount, String sMaterialType) {
        ...
        try {
            ...
            Object evalResult = engine.eval(sFormula);
            double dEvalAmt = (evalResult != null && !"NaN".equals(evalResult.toString()))
                    ? Double.parseDouble(evalResult.toString()) : 0.0;
...
/*        double dActualAmt = Double.parseDouble(txtToGive.getText());
    ...
    private void btSaveOnAction(ActionEvent event) {
        ...
        if(forAdd) {
            ...
            try {                
                ...
                if(this.closedbOp.deleteDenomination(sOperation, sBillNumber, sMaterialType)) {
                    if(this.closedbOp.saveDenominationValues(sOperation, sBillNumber, sMaterialType, currencyListToSave)) {
                    }
                }
            } catch (Exception ex) {
        ...
        if(gParent != null) {
            gParent.sRebilledNewAmt = txtOpenNewAmt.getText();
            gParent.sNewWeight = txtTotalWeight.getText();
            gParent.dialog.close();
        } else {
            sParent.sRebilledNewAmt = txtOpenNewAmt.getText();
            sParent.sNewWeight = txtTotalWeight.getText();
            sParent.dialog.close();
        }                
    }
    ...
    @FXML
    private void txtNewWeightOnAction(ActionEvent event) {        
        Platform.runLater(() -> {
            txtOpenNewAmt.requestFocus();
            txtOpenNewAmt.positionCaret(txtAmtGotInHand.getText().length());
        });                                
    }

    @FXML
    private void txtWeightOnPress(KeyEvent e) {
        
        if(e.getCode() == KeyCode.BACK_SPACE){ 
            
            String sWeight;
            
            if(txtNewWeight.getCaretPosition() == 0) {
                StringBuilder sb = new StringBuilder(txtNewWeight.getText());
                sWeight = sb.toString();
            } else {
                StringBuilder sb = new StringBuilder(txtNewWeight.getText());
                sb.deleteCharAt(txtNewWeight.getCaretPosition() - 1);
                sWeight = sb.toString();
            }
            
            if(!"".equals(sWeight) && Double.parseDouble(sWeight) > 0) 
            {
                double dWeight = Double.parseDouble(sWeight);
                setWeightRelatedWork(dWeight);
            } else {
                txtBalanceToGive.setText("");
            }
        }                
    ...
    @FXML
    private void txtWeightOnType(KeyEvent e) {
        ...
        if("0123456789.".contains(e.getCharacter())){ 
            String sWeight;
            if(txtNewWeight.getCaretPosition() == txtNewWeight.getText().length()) {
                sWeight = txtNewWeight.getText() + e.getCharacter(); 
            } else {
                StringBuilder sb = new StringBuilder(txtNewWeight.getText());
                sb.insert(txtNewWeight.getCaretPosition(), e.getCharacter());
                sWeight = sb.toString();
            }
            double dWeight = Double.parseDouble(sWeight);
            setWeightRelatedWork(dWeight);
    ...
    private void setWeightRelatedWork(double dNewWt) {
        
        double dExistWt = Double.parseDouble(txtExistingWeight.getText());
        double dTotWt = 0;
        
        if(this.sStatus.equals("REBILLED-ADDED")) {
            dTotWt = dExistWt + dNewWt;
        } else if(this.sStatus.equals("REBILLED-REMOVED")) {
            dTotWt = dExistWt - dNewWt;
        }           
        this.sGrossWeight = Double.toString(dTotWt);
        ...
        txtTotalWeight.setText(String.format("%.3f", dTotWt));  
        setAmountRelatedText(Double.parseDouble(txtOpenNewAmt.getText()), gParent != null ? "GOLD" : "SILVER");   
    }
}
