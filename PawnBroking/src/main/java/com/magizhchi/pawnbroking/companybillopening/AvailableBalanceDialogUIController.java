...
public class AvailableBalanceDialogUIController implements Initializable {
    ...
    @FXML
    private TextField txtToGiveAmt;
    @FXML
    private TextField txtGivenAmt;
    @FXML
    private TextField txtCashToGiveAmt;
    @FXML
    private TextField txtCashGivenAmt;
    @FXML
    private Label lbToGetAmt;
    @FXML
    private Label lbGotAmt;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        txtCashToGiveAmt.setEditable(false);
        txtCashToGiveAmt.setMouseTransparent(true);
        txtCashToGiveAmt.setFocusTraversable(false);                
    ...
    public void setParent(GoldBillOpeningController parent, boolean forNew)
    {
        this.gParent = parent;
        this.forAdd = forNew;
        this.toGiveAmt = parent.txtGivenAmount.getText();
        this.sLastSelectedId = parent.sLastSelectedId;
        this.dbOp = parent.dbOp;
        this.sOperation = CommonConstants.G_BILL_OPENING_OPERATION;
        this.sMaterialType = "GOLD";
    }

    public void setParent(SilverBillOpeningController parent, boolean forNew)
    {
        this.sParent = parent;
        this.forAdd = forNew;        
        this.toGiveAmt = parent.txtGivenAmount.getText();
        this.sLastSelectedId = parent.sLastSelectedId;
        this.dbOp = parent.dbOp;
        this.sOperation = CommonConstants.S_BILL_OPENING_OPERATION;
        this.sMaterialType = "SILVER";
    }
    
    public void setInitValues(List<AvailableBalanceBean> existingCurrencyList, 
            boolean isMultiClose, boolean isMultiOperation) throws SQLException {
        
        this.isMultiClose = isMultiClose;
        this.isMultiOperation = isMultiOperation;

        if(forAdd) {
            txtCashGivenAmt.textProperty().addListener(new ChangeListener<String>() {
                @Override
                public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                   if (!newValue.matches("\\d{0,9}([\\.]\\d{0,2})?")) {
                        txtCashGivenAmt.setText(oldValue);
                        double dDeficitAmt =  Double.parseDouble(oldValue) - 
                                Double.parseDouble(txtCashToGiveAmt.getText());
                        ...
                        if(!"".equals(newValue)) {
                            double dDeficitAmt =  Double.parseDouble(newValue) - 
                                    Double.parseDouble(txtCashToGiveAmt.getText().isEmpty() ? "0" 
                                            : txtCashToGiveAmt.getText());
                            if(dDeficitAmt <= 0) {
                                lbCashBalance.setText("Balance To Give:");
                                txtCashBalanceAmt.setText(Double.toString(dDeficitAmt).replace("-", ""));
                            } else {
        ...
        txtCashToGiveAmt.setText(toGiveAmt);
        txtToGiveAmt.setText(toGiveAmt);     
        
        if(isMultiOperation) {
            double totalAmtToGet = 0;
            String multiOpBillNumber = sLastSelectedId + "-OPEN";
            String[] multiBillDetails = 
                    dbOp.getDenominationDetails(CommonConstants.MULTI_OPERATION_OPERATION, 
                            multiOpBillNumber, sMaterialType);                       
            String[] billNumbers = multiBillDetails[0].split(",");
            String[] materialTypes = multiBillDetails[1].split(",");
            for(int i=0; i<billNumbers.length; i++) {
                String[] billNumAndOperation = billNumbers[i].split("-");
                if(billNumAndOperation[1].equals("CLOSE")) {
                    totalAmtToGet = totalAmtToGet + dbOp.getReceivedAmount(billNumAndOperation[0], materialTypes[i]);
                } else if(billNumAndOperation[1].equals("OPEN")) {
                    totalAmtToGet = totalAmtToGet - dbOp.getGivenAmount(billNumAndOperation[0], materialTypes[i]);
                }
            }
            toGiveAmt = Double.toString(totalAmtToGet);
            if(totalAmtToGet <= 0) {
                lbToGetAmt.setText("Amount to Give:");
                lbGotAmt.setText("Given Amount:");
                txtCashToGiveAmt.setText(toGiveAmt.replace("-", ""));
                txtToGiveAmt.setText(toGiveAmt.replace("-", ""));
            } else {
                lbToGetAmt.setText("Amount to Get:");
                lbGotAmt.setText("Received Amount:");
                txtCashToGiveAmt.setText(toGiveAmt);
                txtToGiveAmt.setText(toGiveAmt);            
            }  
            
            double deficit = Double.parseDouble(txtGivenAmt.getText()) - Double.parseDouble(txtToGiveAmt.getText());
            if(lbToGetAmt.getText().equals("Amount to Get:")) {
                if(deficit <= 0) {
            ...
            } else if(lbToGetAmt.getText().equals("Amount to Give:")) {
                if(deficit <= 0) {
                    lbBalance.setText("Balance To Give:");
                    txtBalanceAmt.setText(Double.toString(deficit).replace("-", ""));
                    lbCashBalance.setText("Balance To Give:");
    ...
    public void setValuesToFileds(int index, int numberOfNotes) {
        ...
        double deficit = totAmt - Double.parseDouble(toGiveAmt);    
        if(deficit <= 0) {
            lbBalance.setText("Balance To Give:");
            txtBalanceAmt.setText(Double.toString(deficit).replace("-", ""));
            lbCashBalance.setText("Balance To Give:");
        ...
        txtGivenAmt.setText(Double.toString(totAmt));
        txtCashGivenAmt.setText(Double.toString(totAmt));
    }
        ...
        if(forAdd) {
            ...
            try {                
                ...
                if(isMultiOperation) {
                    ...
                    sBillNumber = this.sLastSelectedId;               
                }
    ...
    private void tabDenominationSelectionChanged(Event event) {
        if(tabPaneScreen.getSelectionModel().isSelected(0) && vbNumberOfNotes.getChildren().size() > 1) {
            Platform.runLater(() -> {
                TextField TF = (TextField)vbNumberOfNotes.getChildren().get(1);
                TF.requestFocus();
