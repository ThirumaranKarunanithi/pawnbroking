 ...
 * @author Tiru
 */
public class RepledgeCreditController implements Initializable {
    
    private RepledgeCreditDBOperation dbOp;
    ...
    private final String BILLING_SCREEN_NAME = "REPLEDGE_BILL_CREDIT";
    private final String OTHER_SCREEN_NAME = "REPLEDGE_OTHER_CREDIT";
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            ...
            try {
                dbOp = new RepledgeCreditDBOperation(CommonConstants.DB, CommonConstants.IP, CommonConstants.PORT, CommonConstants.SCHEMA, CommonConstants.DB_USERNAME, CommonConstants.DB_PASSWORD);
            } catch (ClassNotFoundException ex) {
                Logger.getLogger(RepledgeCreditController.class.getName()).log(Level.SEVERE, null, ex);
            }
        ...
        try {
            ...
            if(dbOp.getAddViewUpdate(CommonConstants.ROLEID, CommonConstants.OPERATION_TAB, CommonConstants.REPLEDGE_INCOME_SCREEN, "ALLOW_ADD") || CommonConstants.ROLEID.equals(CommonConstants.TIRU)) {
    ...
    private void btROSaveBillClicked(ActionEvent event) {
        ...
        if(DateRelatedCalculations.isFirstDateIsLesserToSecondDate(CommonConstants.ACTIVE_COMPANY_ACC_LAST_DATE, sDebittedDate))
        {
            ...
            if(isValidROHeaderValues(sName, sReason, sDebittedAmount)) {
                try {
                    if(dbOp.saveROBillCredit(sId, sRepledgeId, sRepledgeName, sDebittedDate, sExpenseType, sName, sReason, sNote, sInvoiceNo, dInvoiceAmount, dDebittedAmount)) {
                        dbOp.setNextId(OTHER_SCREEN_NAME, CommonConstants.REP_CREDIT_RO_PREFIX + (Integer.parseInt(sId.replace(CommonConstants.REP_CREDIT_RO_PREFIX, ""))+1));
    ...
    private void btSaveBillClicked(ActionEvent event) {
        ...
        if(DateRelatedCalculations.isFirstDateIsLesserToSecondDate(CommonConstants.ACTIVE_COMPANY_ACC_LAST_DATE, sDebittedDate))
        {
            ...
            if(isValidHeaderValues(sRepledgeId, sRepledgeName, sBillNumber, sDebittedAmount)) {
                try {
                    if(dbOp.saveBillCredit(sId, "GOLD", sRepledgeBillNumber, sRepledgeBillStatus, sBillNumber, sStatus, dRepledgeBillAmount, dBillAmount, sDebittedDate, dDebittedAmount, dDebittedAmount, sNote, sRepledgeId, sRepledgeName)) {
                        dbOp.setNextId(BILLING_SCREEN_NAME, CommonConstants.REP_CREDIT_RB_PREFIX + (Integer.parseInt(sId.replace(CommonConstants.REP_CREDIT_RB_PREFIX, ""))+1));
                        PopupUtil.showInfoAlert(event, "Amount Rs."+sDebittedAmount+" debitted for bill "+ sBillNumber +" in "+sRepledgeName+".");
                        txtDebitId.setText(dbOp.getId(BILLING_SCREEN_NAME));
                        btClearAllClicked(event);
                    } 
                } catch (Exception ex) {
                    Logger.getLogger(CompanyDebitController.class.getName()).log(Level.SEVERE, null, ex);
                }
    ...
    public void doAllSaveModeOFFWork() {
        ...
        txtDebitId.setText(CommonConstants.REP_CREDIT_RB_PREFIX);
    ...
    public void doROAllSaveModeOFFWork() {
        ...
        txtRODebitId.setText(CommonConstants.REP_CREDIT_RO_PREFIX);
