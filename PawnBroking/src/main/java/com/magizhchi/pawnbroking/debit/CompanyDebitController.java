 ...
 * @author Tiru
 */
public class CompanyDebitController implements Initializable {

    private CompanyDebitDBOperation dbOp;
    private String sLastSelectedId = null;
    private String sReduceForId = null;
    private String sEOLastSelectedId = null;
    
    private final String BILLING_SCREEN_NAME = "COMPANY_BILL_DEBIT";
    private final String OTHER_SCREEN_NAME = "COMPANY_OTHER_DEBIT";
    ...
    @FXML
    private ComboBox<String> cbEOExpenseOrAsset;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            ...
            try {
                dbOp = new CompanyDebitDBOperation(CommonConstants.DB, CommonConstants.IP, CommonConstants.PORT, CommonConstants.SCHEMA, CommonConstants.DB_USERNAME, CommonConstants.DB_PASSWORD);
            ...
            txtReduceForIdPrefix.setText(CommonConstants.COMP_CREDIT_CB_PREFIX);
        ...
        try {
            ...
            if(dbOp.getAddViewUpdate(CommonConstants.ROLEID, CommonConstants.OPERATION_TAB, CommonConstants.COMPANY_EXPENSES_SCREEN, "ALLOW_ADD") || CommonConstants.ROLEID.equals(CommonConstants.TIRU)) {
            ...
            if(cbBillMaterialType.getValue().toString().equals("GOLD")) {
                billRowAndNumber = dbOp.getGoldCurrentBillNumber();
            } else if(cbBillMaterialType.getValue().toString().equals("SILVER")) {
    ...
    public void setCompanyExpenseTypeValues() {
        Platform.runLater(()->{
            try {
                ...
                List<String> lst = dbOp.getCompanyExpenseList("COMPANY EXPENSES");
                if(!lst.isEmpty()) {
    ...
    private void capitalizeCharOnType(KeyEvent e) {
        // Use runLater so getText() is read AFTER JavaFX inserts the typed character
        Platform.runLater(() -> {
    ...
    private void capitalizeCharOnPressed(KeyEvent e) {
        if (e.getCode() == KeyCode.BACK_SPACE) {
            // Let JavaFX apply the backspace, then uppercase the result
            Platform.runLater(() -> {
                TextField txt_TextField = (TextField) e.getSource();
                int caretPos = txt_TextField.getCaretPosition();
                txt_TextField.setText(txt_TextField.getText().toUpperCase());
    ...
    private void txtBillNumberOnAction(ActionEvent event) {
        ...
        } else         
        {
            ...
            try {
                HashMap<String, String> headerValues = dbOp.getAllBillingHeaderValuesByBillNumber(sBillNumber, sMaterialType);
                ...
                if(headerValues != null)
                {
                    ...
                    cbAction.setValue(headerValues.get("ID_ACTION"));
                    cbStatus.setValue(headerValues.get("ID_STATUS"));
                    String creId = headerValues.get("CREDIT_ID_FOR");
                    txtReduceForIdPrefix.setText(CommonConstants.COMP_CREDIT_CB_PREFIX);
                    txtReduceForId.setText(creId.replace(CommonConstants.COMP_CREDIT_CB_PREFIX, ""));
    ...
    public void setAllHeaderValuesToFields(HashMap<String, String> headerValues)
    {
            ...
            if("CLOSED".equals(sStatus) || "DELIVERED".equals(sStatus)) {
        ...
        if(sLastSelectedId != null) {
            ...
            {            
                ...
                if(isValidHeaderValues(sDebit)) {
                    try {
                        if(dbOp.saveBillDebit(sId, sMaterialType, sLastSelectedId, 
                                sStatus, dBillAmount, sDebittedDate, dToDebit, dDebit, sNote,
                                sIdAction, sIdStatus, sCreditIdFor)) {
                            dbOp.setNextId(BILLING_SCREEN_NAME, CommonConstants.COMP_DEBIT_CB_PREFIX + (Integer.parseInt(sId.replace(CommonConstants.COMP_DEBIT_CB_PREFIX, ""))+1));
                            PopupUtil.showInfoAlert(event, "Amount Rs."+dDebit+" debitted for "+sMaterialType+" Bill No: "+ sLastSelectedId +".");
                            txtDebitId.setText(dbOp.getId(BILLING_SCREEN_NAME));
                            btClearAllClicked(null);
    ...
    private void btEOSaveBillClicked(ActionEvent event) {
        ...
        if(DateRelatedCalculations.isFirstDateIsLesserToSecondDate(CommonConstants.ACTIVE_COMPANY_ACC_LAST_DATE, sDebittedDate))
        {
            ...
            String sExpenseOrAsset = cbEOExpenseOrAsset.getValue();
            ...
            if(isValidCOHeaderValues(sName, sReason, sDebittedAmount)) {
                try {
                    if(dbOp.saveCOBillDebit(sId, sDebittedDate, sExpenseType, 
                            sName, sReason, sNote, sInvoiceNo, dInvoiceAmount, 
                            dDebittedAmount, sExpenseOrAsset)) {
                        dbOp.setNextId(OTHER_SCREEN_NAME, 
                                CommonConstants.COMP_DEBIT_CO_PREFIX 
                                        + (Integer.parseInt(sId.replace(CommonConstants.COMP_DEBIT_CO_PREFIX, ""))+1));
                        PopupUtil.showInfoAlert(event, "Amount Rs."+sDebittedAmount+" debitted for "+sExpenseType+".");
                        txtEODebitId.setText(dbOp.getId(OTHER_SCREEN_NAME));
    ...
    public void doAllSaveModeOFFWork() {
        ...
        txtDebitId.setText(CommonConstants.COMP_DEBIT_CB_PREFIX);
    ...
    private void btUpdateBillClicked(ActionEvent event) {
        ...
        if(sLastSelectedId != null) {
            ...
            if(DateRelatedCalculations.isFirstDateIsLesserToSecondDate(CommonConstants.ACTIVE_COMPANY_ACC_LAST_DATE, sDebittedDate))
            {
                ...
                if(isValidHeaderValues(sDebit)) {
                    try {
                        if(dbOp.updateBillDebit(sId, sDebittedDate, dDebit, sNote, sIdAction, sIdStatus, sCreditIdFor)) {
    ...
    private void txtDebitIdOnAction(ActionEvent event) {
        ...
        if(tgOff.isSelected()) 
        {
            ...
            try {
                ...
                if(headerValues != null)
                {
                    txtDebitId.setText(headerValues.get("DEBIT_ID"));       
                    dpDebittedDate.setValue(LocalDate.parse(headerValues.get("DEBITTED_DATE"), CommonConstants.DATETIMEFORMATTER));
                    txtToGiveAmount.setText(headerValues.get("TOGIVE_AMOUNT"));
                    txtGivenAmount.setText(headerValues.get("GIVEN_AMOUNT"));
    ...
    public void doEOAllSaveModeOFFWork() {
        ...
        txtEODebitId.setText(CommonConstants.COMP_DEBIT_CO_PREFIX);
        ...
        if(tgEOOff.isSelected()) 
        {
            ...
            try {
                ...
                if(headerValues != null)
                {
                    ...
                    cbEOExpenseOrAsset.setValue(headerValues.get("expense_or_asset"));
    ...
    private void txtReduceForIdOnAction(ActionEvent event) {
        ...
        if(tgOn.isSelected()) 
        {
            ...
            try {
                HashMap<String, String> headerValues = dbOp.getReducedForCompanyBillCreditValues(sCreditId);
    ...
    @FXML
    private void cbBillMaterialTypeOnAction(ActionEvent event) {
    ...
    private void cbEOExpenseTypeOnAction(ActionEvent e) {
        ...
        Platform.runLater(()->{        
            CommonConstants.CURRENT_OPERATION = CommonConstants.COMPANY_EXPENSES;
    ...
    @FXML
    private void cbEOExpenseOrAssetOnAction(ActionEvent event) {
    }
