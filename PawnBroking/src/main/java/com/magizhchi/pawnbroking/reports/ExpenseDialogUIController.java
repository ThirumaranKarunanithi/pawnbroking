 ...
 * @author Tiru
 */
public class ExpenseDialogUIController implements Initializable {
    ...
    public void setInitValues(DataTable values) {
        ...
        for(int i=0; i<values.getRowCount(); i++) {            
            ...
            if(sScreenName.equals("EMPLOYEE SALARY AMOUNT")) {
                String[] idAndName = values.getRow(i).getColumn(3).toString().split(" - ");
                try {
                    sReason = idAndName[1] + " (Total Salary: " + this.parent.dbOp.getEmployeeTotalSal(idAndName[0])
                            + ", Reduced Total Amount: " + this.parent.dbOp.getEmployeeAdvanceAmountReducedBySalId(sExpenseId) + ")\n"
                            + "(" + this.parent.dbOp.getEmployeeAdvanceAmountDetails(sExpenseId) + ")";
