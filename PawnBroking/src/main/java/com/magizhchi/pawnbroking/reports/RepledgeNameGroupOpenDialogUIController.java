 ...
 * @author tiruk
 */
public class RepledgeNameGroupOpenDialogUIController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tbTodaysAccount.setRowFactory(tv -> new TableRow<RepledgeNameGroup>() {
            ...
            public void updateItem(RepledgeNameGroup item, boolean empty) {
                ...
                if (item == null) {
                    setStyle("");
                } else if (item.getOpenCapitalAmount() > 1000000) {
    ...
    public void setInitValues(DataTable values) {
        ...
        for(int i=0; i<values.getRowCount(); i++) {            
            ...
            String sOpenCapitalAmount = values.getRow(i).getColumn(2).toString();
            String sOpenInterestSpent = values.getRow(i).getColumn(3).toString();
            tbTodaysAccount.getItems().add(
                    new RepledgeNameGroup(
                            Integer.parseInt(sSlNo), 
                            sRepledgeName, 
                            Integer.parseInt(sCount), 
                            Double.parseDouble(sOpenCapitalAmount), 
                            Double.parseDouble(sOpenInterestSpent)));
        }
    }
