...
public class BillController {
    ...
    public BillController(@Qualifier("cloudJdbcTemplate") JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
     ...
     *               search (bill number or customer name), page, size
     */
    @GetMapping
    public ResponseEntity<Map<String, Object>> list(
        ...
        if (!search.isBlank()) {
            where.append(" AND (bill_number ILIKE ? OR customer_name ILIKE ?)");
            params.add("%" + search + "%");
            params.add("%" + search + "%");
        }
